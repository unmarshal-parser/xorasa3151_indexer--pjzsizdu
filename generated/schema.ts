...
export class NewMint extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save NewMint entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type NewMint must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("NewMint", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): NewMint | null {
    return changetype<NewMint | null>(
      store.get_in_block("NewMint", id.toHexString())
    );
  }

  static load(id: Bytes): NewMint | null {
    return changetype<NewMint | null>(store.get("NewMint", id.toHexString()));
  }
  ...
  get mintQuantity(): BigInt {
    let value = this.get("mintQuantity");
    if (!value || value.kind == ValueKind.NULL) {
      throw new Error("Cannot return null for a required field.");
    } else {
      return value.toBigInt();
  ...
  set mintQuantity(value: BigInt) {
    this.set("mintQuantity", Value.fromBigInt(value));
  }
